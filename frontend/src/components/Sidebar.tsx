...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed top-0 left-0 bottom-0 w-[220px] bg-bg border-r border-line flex flex-col z-drawer
      ...
      `}>
        ...
        <div className="px-5 py-6 border-b border-line flex items-center gap-2.5">
          ...
          <span className="font-brand font-bold text-[15px] tracking-widest text-content-strong">{t('brand.name')}</span>
          <span className="ml-auto text-[9px] text-content-muted bg-surface-2 px-1.5 py-0.5 rounded border border-line-2 tracking-widest uppercase">{t('brand.plan')}</span>
        </div>

        {/* Nav */}
        <div className="p-3 flex flex-col gap-0.5 mt-2">
          <p className="text-[10px] text-content-faint uppercase tracking-widest px-2 pb-2">{t('nav.overview')}</p>
          ...
          <NavLink to="/" end onClick={onClose} className={({ isActive }) =>
            `flex items-center gap-2.5 px-2.5 py-2 rounded-md text-[13px] transition-all border ${
              isActive
                ...
                : 'text-content-muted border-transparent hover:bg-surface-3 hover:text-content-secondary'
            }`
          }>
          ...
          <p className="text-[10px] text-content-faint uppercase tracking-widest px-2 pb-2 mt-4">{t('nav.trades')}</p>
        ...
        <div className="mt-auto p-3 border-t border-line">
          <NavLink to="/profile" onClick={onClose} className={({ isActive }) =>
            ...
            {profile?.avatarUrl ? (
              ...
              <div className="w-[30px] h-[30px] rounded-full bg-brand flex items-center justify-center text-[11px] font-bold shrink-0">
                {initials(profile?.displayName ?? name)}
              </div>
            )}
            <div className="flex-1 min-w-0">
              <div className="text-xs text-content-strong truncate">{profile?.displayName || name || 'Trader'}</div>
              <div className="text-[10px] text-content-muted truncate">{profile?.email ?? ''}</div>
            </div>
            <div className="w-1.5 h-1.5 rounded-full bg-pos shrink-0" />
          </NavLink>
        </div>
